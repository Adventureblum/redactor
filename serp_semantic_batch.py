...
class SerpFileProcessor:
    ...
    def _extract_keywords_from_agent_data(self, agent_data: Dict, main_keyword: str) -> str:
        """Extrait et génère des mots-clés depuis les données agent_response"""
        try:
            keywords = []
            ...
            # Extraire des mots-clés depuis les différentes sections
            shock_stats = agent_data.get('shock_statistics', [])
            for stat in shock_stats[:3]:  # Prendre les 3 premières statistiques
                statistic_text = stat.get('statistic', '')
                # Extraire des termes pertinents de la statistique
                terms = self._extract_terms_from_text(statistic_text)
                keywords.extend(terms[:5])

            # Ajouter des termes depuis expert_insights
            ...
            # Ajouter des termes depuis market_trends
            ...
            # Ajouter des termes depuis content_marketing_angles
            ...
            logging.warning(f"Erreur lors de l'extraction des mots-clés depuis agent_data : {e}")
            return ""

    def _extract_terms_from_text(self, text: str) -> List[str]:
        """Extrait des termes pertinents depuis un texte"""
        try:
            # Nettoyer le texte
            text = re.sub(r'[^\w\s]', ' ', text)
            words = text.split()

            # Filtrer les mots pertinents (longueur > 3, pas de chiffres purs)
            terms = []
            for word in words:
                if len(word) > 3 and not word.isdigit() and word.lower() not in {'pour', 'avec', 'dans', 'plus', 'sont', 'cette', 'leurs', 'tout'}:
                    terms.append(word)
    ...
    def _generate_angles_from_agent_data(self, agent_data: Dict, main_keyword: str, enriched_context: Dict) -> List[str]:
        """Génère des angles différenciants basés sur les données agent_response"""
        try:
            angles = []

            # 1. Angles basés sur shock_statistics
            shock_stats = agent_data.get('shock_statistics', [])
            for stat in shock_stats[:3]:
                statistic = stat.get('statistic', '')
                context = stat.get('context', '')
                if statistic:
                    angle = f"**Analyse Statistique : {statistic}** - {context if context else 'Données factuelles pour étayer votre décision concernant ' + main_keyword}"
                    angles.append(angle)

            # 2. Angles basés sur expert_insights
            expert_insights = agent_data.get('expert_insights', [])
            for insight in expert_insights[:2]:
                insight_text = insight.get('insight', '')
                authority = insight.get('authority_source', '')
                if insight_text:
                    angle = f"**Perspective d'Expert ({authority}) : {main_keyword}** - {insight_text}"
                    angles.append(angle)

            # 3. Angles basés sur market_trends
            market_trends = agent_data.get('market_trends', [])
            for trend in market_trends[:2]:
                trend_text = trend.get('trend', '')
                projection = trend.get('future_projection', '')
                if trend_text:
                    angle = f"**Tendance Marché : {trend_text}** - {projection if projection else 'Impact sur ' + main_keyword}"
                    angles.append(angle)

            # 4. Angles basés sur competitive_landscape
            ...
            for comp in competitive[:2]:
                ...
                if comparison:
                    angle = f"**Comparatif Concurrentiel : {comparison}** - {difference if difference else 'Analyse comparative pour ' + main_keyword}"
                    angles.append(angle)

            # 5. Utiliser content_marketing_angles directement si disponibles
            marketing_angles = agent_data.get('content_marketing_angles', [])
            for marketing_angle in marketing_angles[:2]:
                if marketing_angle:
                    angle = f"**Angle Marketing : {main_keyword}** - {marketing_angle}"
                    angles.append(angle)

            # Nettoyer et limiter les angles
            cleaned_angles = [angle for angle in angles if angle and len(angle) > 20]
            return cleaned_angles[:10]

        except Exception as e:
            logging.warning(f"Erreur lors de la génération d'angles depuis agent_data : {e}")
            return []
    ...
    def _build_final_result(self, main_keyword: str, refined_keywords: str, differentiating_angles: List[str], max_word_count: int, enriched_context: Dict) -> Dict:
        ...
        return {
            ...
            'semantic_analysis': {
                'entities': [ent["nom"] for ent in enriched_context.get("entites_importantes", [])[:5]],
                'clusters_count': enriched_context.get("statistiques_semantiques", {}).get("nombre_clusters", 0),
                'relations_found': enriched_context.get("statistiques_semantiques", {}).get("nombre_relations", 0),
                'thematic_diversity': enriched_context.get("statistiques_semantiques", {}).get("diversite_thematique", 0),
                'semantic_complexity': enriched_context.get("statistiques_semantiques", {}).get("complexite_semantique", 0)
            },
            'differentiating_angles': differentiating_angles
        }
    def detect_search_intention(self, query: str) -> str:
    ...
    def calculate_topic_complexity(self, tfidf_scores: Dict, entities: List[Dict], 
    ...
    def select_sections_by_matrix(self, intention: str, complexity: str) -> Dict[str, Dict]:
    ...
    def calculate_word_distribution(self, sections: Dict[str, Dict], target_word_count: int) -> Dict[str, int]:
    ...
    def generate_section_metadata(self, intention: str, sections_config: Dict, 
    ...
    async def process_file(self, filepath: str, query_data: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            # === NOUVEAU : Récupération prioritaire des données agent_response ===
            ...
            if has_agent_data:
                logging.info(f"✓ Données agent_response trouvées pour {main_keyword}, utilisation en priorité")
            else:
                logging.info(f"ℹ️ Aucune donnée agent_response pour {main_keyword}, fallback sur analyse SERP classique")
            ...
            # === 5. Génération intelligente avec priorité agent_response ===
            refined_keywords = ""
            differentiating_angles = []

            # Utilisation prioritaire des données agent_response si disponibles
            if has_agent_data:
                try:
                    logging.info(f"🎯 Utilisation des données agent_response pour enrichir l'analyse")

                    # Extraction des mots-clés depuis agent_response
                    ...
                    if keywords_from_agent:
                        ...
                        logging.info(f"✓ Mots-clés extraits depuis agent_response")

                    # Génération d'angles basés sur agent_response
                    ...
                    if angles_from_agent:
                        ...
                        logging.info(f"✓ Angles générés depuis agent_response ({len(angles_from_agent)} angles)")

                    # Si on a obtenu les deux, on peut passer à la construction du résultat avec structure
                    if refined_keywords and differentiating_angles:
                        logging.info(f"✓ Génération complète via agent_response pour {os.path.basename(filepath)}")

                        # === GÉNÉRATION DE LA STRUCTURE AVANT LE RETURN ===
                        # Détection de l'intention de recherche
                        search_intention = self.detect_search_intention(main_keyword)
                        print(f"🎯 INTENTION DÉTECTÉE pour '{main_keyword}': {search_intention}")

                        # Calcul de la complexité du sujet
                        topic_complexity = self.calculate_topic_complexity(
                            weighted_scores, entities, relations, main_keyword
                        )
                        print(f"🧠 COMPLEXITÉ CALCULÉE pour '{main_keyword}': {topic_complexity}")

                        # Sélection des sections basée sur intention × complexité
                        sections_config = self.select_sections_by_matrix(search_intention, topic_complexity)
                        print(f"📋 SECTIONS SÉLECTIONNÉES: {sections_config}")

                        # Calcul de la distribution des mots par section
                        word_distribution = self.calculate_word_distribution(sections_config, max_word_count)
                        print(f"📊 DISTRIBUTION DES MOTS: {word_distribution}")

                        # Génération des métadonnées de section
                        section_metadata = self.generate_section_metadata(
                            search_intention, sections_config, word_distribution
                        )
                        print(f"📝 MÉTADONNÉES GÉNÉRÉES: {len(section_metadata)} sections configurées")

                        logging.info(f"✓ Structure de contenu générée - Intention: {search_intention}, Complexité: {topic_complexity}")

                        # Construction du résultat final avec structure
                        result = self._build_final_result(main_keyword, refined_keywords, differentiating_angles, max_word_count, enriched_context)

                        # Ajout des données de structuration
                        content_structure = {
                        ...
                        print(f"🏗️ STRUCTURE DE CONTENU AJOUTÉE au résultat pour '{main_keyword}': {len(content_structure)} éléments")
                        print(f"   - Intention: {search_intention}")
                        print(f"   - Complexité: {topic_complexity}")
                        print(f"   - Sections titulaires: {sections_config.get('titulaires', [])}")
                        print(f"   - Sections remplaçantes: {sections_config.get('remplacants', [])}")

                        return result

                except Exception as e:
                    logging.warning(f"Erreur lors de l'utilisation d'agent_response : {e}, fallback sur méthode classique")
                    has_agent_data = False  # Force le fallback

            # Fallback sur la méthode classique si pas d'agent_response ou erreur
            try:
                if not enriched_context["clusters_thematiques"]:
                    ...
                    # Préparer le contexte enrichi avec agent_response si disponible
                    enhanced_context = context_str
                    if has_agent_data:
                        agent_context = json.dumps(agent_response_data, ensure_ascii=False, indent=2)
                        enhanced_context = f"DONNÉES PRIORITAIRES (agent_response):\n{agent_context}\n\nDONNÉES SERP COMPLÉMENTAIRES:\n{context_str}"
                        ...
                        messages=[
                            {
                                "role": "system",
                                "content": (
                                    "Tu es un expert en SEO et en analyse sémantique. "
                                    "PRIORITÉ : Si des données agent_response sont fournies, utilise-les en priorité car elles contiennent des informations factuelles et statistiques vérifiées. "
                                    "Analyse le corpus et retourne 60 mots-clés stratégiques qui couvrent tous les aspects importants du sujet. "
                                    "Intègre en priorité les termes présents dans les statistiques choc, insights d'experts, et tendances marché. "
                                    "Organise-les logiquement et retourne uniquement la liste séparée par des virgules."
                                )
                            },
                            {
                                "role": "user",
                        ...
                        messages=[
                            {
                                ...
                                "content": (
                                    ...
                                    "PRIORITÉ : Si des données agent_response sont fournies, utilise-les en priorité car elles contiennent des statistiques vérifiées, insights d'experts, et tendances marché authentiques. "
                                    "Exploite particulièrement les shock_statistics, expert_insights, market_trends et competitive_landscape pour créer des angles factuels et crédibles. "
                                    "À partir de ces données enrichies, identifie 10 angles différenciants et originaux pour traiter ce sujet. "
                                    "Chaque angle doit exploiter les insights factuels pour se démarquer de la concurrence avec des données concrètes. "
                                    "Format : liste numérotée avec titre et explications succinte mais tu ne doit surtout pas couper des phrases."
                                )
                            },
                            {
                                "role": "user",
                                "content": (
                                    ...
                                    "3. Exploitent les statistiques choc et données factuelles (agent_response prioritaire)\n"
                                    "4. Intègrent les insights d'experts et autorités mentionnées\n"
                                    "5. Utilisent les tendances marché et projections futures\n"
                                    "6. Couvrent les aspects concurrentiels et comparatifs\n"
                                    "7. Exploitent les relations sémantiques et entités découvertes\n\n"
                                    f"Chaque angle doit expliquer comment il répond spécifiquement à '{main_keyword}' avec des données concrètes."
                                )
                            }
                        ],
                        temperature=0.8,
            ...
            # === 6. Construction du résultat final avec analyse structurelle ===
            # Détection de l'intention de recherche
            search_intention = self.detect_search_intention(main_keyword)
            print(f"🎯 INTENTION DÉTECTÉE pour '{main_keyword}': {search_intention}")
        ...
        for query in consigne_data.get('queries', []):
            ...
            if query_id in processed_results:
                ...
                query.update({
                    ...
                    'content_structure': content_structure  # Ajout de cette ligne importante !
                })
